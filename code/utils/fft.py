...
def low_pass_filter(image, size, scale):
    fr_image = np.fft.fftshift(np.fft.fft2(image))
    rr, cc = disk((size // 2, size // 2), size * scale // 2, shape=(size, size))
    freqs_mask = np.zeros((size, size))
    freqs_mask[rr, cc] = 1
    return np.fft.ifft2(np.fft.ifftshift(fr_image * freqs_mask)).real
