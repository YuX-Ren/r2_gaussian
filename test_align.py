...
def training(
    ...
    first_iter = 0
    cryo_dataset = EMPIARDataset(
        mrcs=f"cryo_data/empiar-10076/particles.mrcs",
        ctf=f"cryo_data/empiar-10076/ctf.pkl",
        poses=f"cryo_data/empiar-10076/poses.pkl",
        args=args,
        size=256, sign=1 ,
    )
    # normalize the image to 0-1
    cryo_dataset.images = (cryo_dataset.images - cryo_dataset.images.min()) / (cryo_dataset.images.max() - cryo_dataset.images.min())
    # load the first image
    cryo_dataset.images = cryo_dataset.images[0:1]
    # Set up dataset
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Compute loss
        gt_image = cryo_dataset.images[0]
        max_val = gt_image.max()
        min_val = gt_image.min()
        gt_image = (gt_image - min_val) / (max_val - min_val)
        gt_image = resize(gt_image, (256, 256), order=1)
        gt_image = torch.from_numpy(gt_image).cuda().unsqueeze(0)
